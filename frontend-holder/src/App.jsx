...
function Dashboard({user,onLogout}){
  ...
  const [section,setSection]=useState("accounts"); // "accounts" | "leaderboard"
    ...
    <div style={{display:"flex",height:"100vh",background:C.bg,fontFamily:"'IBM Plex Sans',sans-serif",overflow:"hidden"}}>
      ...
      <div style={{width:270,background:C.surface,borderRight:`1px solid ${C.border}`,display:"flex",flexDirection:"column",flexShrink:0}}>
        ...
        <div style={{flex:1,overflowY:"auto"}}>
          {accounts.length===0
            ...
            : accounts.map(acc=>{
              ...
              return(
                <div key={acc.id} onClick={()=>{setOpenAccId(acc.id);setMainTab("notifications");setSection("accounts");}}
        ...
        <div style={{padding:"8px 12px",borderTop:`1px solid ${C.border}`}}>
          ...
          <button onClick={()=>setShowPauseModal(true)} style={{display:"flex",alignItems:"center",gap:8,width:"100%",background:isPaused?"#0A1A10":"none",border:`1px solid ${isPaused?"#065F46":C.border}`,borderRadius:7,padding:"8px 12px",cursor:"pointer",fontFamily:"inherit",marginBottom:8,marginTop:4,transition:"all 0.15s"}}
      ...
      {/* Main */}
      {section==="leaderboard"
        ? <LeaderboardView userRole="holder"/>
        : openAcc
