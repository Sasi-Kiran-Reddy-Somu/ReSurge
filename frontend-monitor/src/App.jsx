...
function Dashboard({user,onLogout}){
  // Sidebar section: "accounts" (my accounts panel) or "holders" (holders panel)
  ...
  const filteredHolders=holders.filter(h=>h.name.toLowerCase().includes(sideSearch.toLowerCase())||h.email.toLowerCase().includes(sideSearch.toLowerCase()));
  ...
  return(
    <div style={{display:"flex",height:"100vh",background:C.bg,fontFamily:"'IBM Plex Sans',sans-serif",overflow:"hidden"}}>
      ...
      <div style={{width:280,background:C.surface,borderRight:`1px solid ${C.border}`,display:"flex",flexDirection:"column",flexShrink:0}}>
        {/* Header */}
        <div style={{padding:"24px 20px 14px",borderBottom:`1px solid ${C.border}`}}>
          <div style={{fontSize:11,color:C.accent,fontWeight:700,letterSpacing:"0.1em",marginBottom:10}}>👁 MONITOR PORTAL</div>
          <div style={{fontWeight:700,fontSize:15,color:C.text,marginBottom:3}}>{user.name}</div>
          <div style={{fontSize:12,color:C.muted}}>{user.email}</div>
        </div>

        {/* Section tabs */}
        <div style={{display:"flex",padding:"8px 12px",gap:6,borderBottom:`1px solid ${C.border}`,flexShrink:0}}>
          {[{k:"accounts",l:"MY ACCOUNTS"},{k:"holders",l:"HOLDERS"}].map(({k,l})=>(
            <button key={k} onClick={()=>{setSection(k);if(k==="holders")setSelectedHolder(null);}} style={{flex:1,background:section===k?"#1A2A40":"none",border:section===k?`1px solid ${C.accent}30`:`1px solid ${C.border}`,borderRadius:7,padding:"7px 0",color:section===k?C.accent:C.muted,fontFamily:"inherit",fontSize:10,fontWeight:700,cursor:"pointer",letterSpacing:"0.05em"}}>{l}</button>
          ))}
        </div>
        ...
        {section==="accounts"&&(
          <>
            <div style={{padding:"10px 20px 6px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
              <div style={{fontSize:10,color:C.dim,letterSpacing:"0.08em"}}>ACCOUNTS ({accounts.length})</div>
            ...
            <div style={{flex:1,overflowY:"auto"}}>
              {accounts.length===0
                ?<div style={{padding:"10px 20px",fontSize:12,color:C.dim}}>No accounts yet. Click + Add.</div>
                :accounts.map(acc=>{
                    ...
                    <div key={acc.id}>
                      ...
                      {isOpen&&(
                        <div style={{background:"#0A0D14",borderBottom:`1px solid ${C.border}20`,padding:"10px 20px 12px"}}>
                          ...
                          <button onClick={e=>{e.stopPropagation();deleteAccount(acc.id);}} disabled={delAccBusy===acc.id}
                            style={{background:"none",border:"1px solid #7F1D1D",color:C.red,cursor:"pointer",fontFamily:"inherit",fontSize:10,padding:"4px 10px",borderRadius:5,fontWeight:600,opacity:delAccBusy===acc.id?0.5:1}}>
        ...
        {section==="holders"&&(
          <>
            ...
            <div style={{padding:"4px 14px 10px",flex:1,overflowY:"auto"}}>
              <div style={{fontSize:10,color:C.dim,letterSpacing:"0.08em",marginBottom:8,paddingLeft:4}}>HOLDERS ({filteredHolders.length})</div>
              <div style={{display:"flex",flexDirection:"column",gap:2}}>
                {filteredHolders.map(h=>(
                  <div key={h.id} onClick={()=>setSelectedHolder(h)}
                    style={{padding:"10px 12px",borderRadius:8,cursor:"pointer",background:selectedHolder?.id===h.id?"#1E3A5F":"none",border:selectedHolder?.id===h.id?`1px solid ${C.accent}30`:"1px solid transparent",transition:"all 0.1s"}}
                    onMouseEnter={e=>{if(selectedHolder?.id!==h.id)e.currentTarget.style.background="#111318";}}
                    onMouseLeave={e=>{if(selectedHolder?.id!==h.id)e.currentTarget.style.background="none";}}>
                    <div style={{fontSize:13,fontWeight:600,color:selectedHolder?.id===h.id?"#93C5FD":C.text,marginBottom:2}}>{h.name}</div>
                    <div style={{fontSize:11,color:C.muted}}>{h.converted} posted · {h.totalNotifications} notified</div>
                  </div>
                ))}
                {filteredHolders.length===0&&<div style={{fontSize:12,color:C.dim,padding:"8px 6px"}}>{holders.length===0?"No holders assigned.":"No results."}</div>}
              </div>
            </div>
          </>
        )}
      ...
      {section==="accounts"
        ...
        : section==="holders" && selectedHolder
