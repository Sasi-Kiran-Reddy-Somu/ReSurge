...
// ── Create Monitor Form ───────────────────────────────────────
function CreateMonitorForm({ onCreated }) {
  ...
  return (
    <form onSubmit={submit} style={{ ...S.card, borderColor:"#1E3A5F" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:10 }}>
        ...
        <div><label style={S.label}>Email</label><input style={S.input} type="email" value={form.email} onChange={e=>setForm({...form,email:e.target.value})} required /></div>
        <div><label style={S.label}>Password <span style={{color:"#374151"}}>(skip if existing user)</span></label><input style={S.input} type="password" value={form.password} onChange={e=>setForm({...form,password:e.target.value})} /></div>
        <div><label style={S.label}>Phone (optional)</label><input style={S.input} value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})} /></div>
      </div>
...
// ── Assign Holder Modal ───────────────────────────────────────
function AssignModal({ monitor, onClose, onAssigned }) {
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:200 }}>
      <div style={{ background:"#0F1117", border:"1px solid #1F2937", borderRadius:12, padding:24, width:360 }}>
        ...
        <select value={selected} onChange={e=>setSelected(e.target.value)}
          ...
          {holders.map(h => <option key={h.id} value={h.id}>{h.name} ({h.email})</option>)}
        </select>
...
function DeleteConfirmModal({ monitor, onClose, onConfirm }) {
  const [busy, setBusy] = useState(false);
...
// ── Monitor Card ──────────────────────────────────────────────
function MonitorCard({ monitor, onRefresh }) {
  ...
  useEffect(() => {
    if (expanded) {
      req("GET", `/admin/monitors/${monitor.id}/assignments`).then(async (assignments) => {
        const all = await req("GET", "/admin/holders");
        const assigned = all.filter(h => assignments.some(a => a.holderId === h.id));
        setHolders(assigned);
      }).catch(() => {});
    }
  }, [expanded, monitor.id]);
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          ...
          <div style={{ display:"flex", gap:6, alignItems:"center" }}>
            ...
            <div style={{ position:"relative" }}>
              <button onClick={() => setShowSettings(s => !s)} style={S.btn("#1F2937","#9CA3AF")}>⚙ Settings</button>
              {showSettings && (
            ...
            <span style={{ cursor:"pointer", color:"#6B7280", fontSize:18, marginLeft:4, lineHeight:1 }} onClick={() => { setExpanded(e => !e); setShowSettings(false); }}>
              {expanded ? "▲" : "▼"}
            </span>
          </div>
        </div>
  ...
  return (
    <div style={{ flex:1, overflowY:"auto", padding:"20px 24px" }}>
      ...
      {filteredMonitors.length === 0
        ...
        : filteredMonitors.map(m => <MonitorCard key={m.id} monitor={m} onRefresh={load} />)
      }
    </div>
  );
}
