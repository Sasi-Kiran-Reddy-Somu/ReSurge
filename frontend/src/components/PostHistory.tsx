...
export default function PostHistory({ subreddit }: any) {
  const [posts,    setPosts]   = useState<any[]>([]);
  const [loading,  setLoading] = useState(true);
  const [search,   setSearch]  = useState("");
  const [time,     setTime]    = useState("all");   // "all"|"today"|"week"|"month"|"custom"
  ...
  function getTimeBounds() {
    ...
    if (time === "custom") {
      ...
      const t = toDate   ? new Date(toDate).getTime() + 86399999 : now; // end of toDate
      return { from: f, to: t };
    }
  ...
  const visible = posts.filter((p: any) => {
    ...
    if (search) {
      ...
      if (!p.title.toLowerCase().includes(q) && !p.subreddit.toLowerCase().includes(q)) return false;
    }
    return true;
  });
  ...
  return (
    <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
          ...
          <div style={{ display:"flex", gap:6 }}>
            {statusBtns.map(({ k, l, c }) => (
              <button key={k} onClick={() => setStatus(k)}
                style={{ background: status===k ? "#111827":"#1F2937", color: status===k ? c:"#6B7280",
                  border: status===k ? `1px solid ${c}40`:"none", borderRadius:6,
                  padding:"7px 14px", fontSize:11, fontWeight: status===k ? 700:400,
        ...
        <div style={{ display:"flex", gap:6, marginBottom:12, flexWrap:"wrap", alignItems:"center" }}>
          {timeBtns.map(([k, l]) => (
            <button key={k} onClick={() => setTime(k)}
              style={{ background: time===k ? "#1C1400":"#1F2937", color: time===k ? "#F59E0B":"#6B7280",
                border: time===k ? "1px solid #78350F":"none", borderRadius:6,
                padding:"7px 14px", fontSize:11, fontWeight: time===k ? 700:400,
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"0 24px 24px" }}>
        {loading ? (
          <div style={{ color:"#6B7280", fontSize:12, padding:"40px 0", textAlign:"center" }}>Loading history...</div>
        ) : visible.length === 0 ? (
          ...
          <div style={{ background:"#0F1117", border:"1px solid #1F2937", borderRadius:12, overflow:"hidden" }}>
            <div style={{ display:"grid", gridTemplateColumns:"140px 1fr 80px 70px 70px 80px", gap:12, padding:"10px 18px", borderBottom:"1px solid #1F2937", position:"sticky", top:0, background:"#0F1117", zIndex:1 }}>
              {["Alerted","Post","Growth","Upvotes","Eng","Status"].map(h => (
                <div key={h} style={{ fontSize:10, color:"#6B7280", fontWeight:700, letterSpacing:"0.05em" }}>{h}</div>
              ))}
            </div>
            {visible.map((p: any) => (
              <div key={p.id}
                style={{ display:"grid", gridTemplateColumns:"140px 1fr 80px 70px 70px 80px", gap:12, padding:"13px 18px", borderBottom:"1px solid #111318", alignItems:"center", transition:"background 0.1s" }}
